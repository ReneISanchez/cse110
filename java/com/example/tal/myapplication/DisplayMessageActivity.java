...
public class DisplayMessageActivity extends ActionBarActivity{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final ArrayList<String> profList = intent.getStringArrayListExtra(MainActivity.EXTRA_PROFLIST);
       ...
       Thread getCapeData = new Thread(new Runnable(){
           ...
           public void run(){
               ...
               try{
                   ...
                   String capeURL =  "http://www.cape.ucsd.edu/responses/Results.aspx?Name=" + profList.get(0) + "&CourseNumber=" + classList.get(0);
                       ...
                       runOnUiThread(new Runnable(){
                           ...
                           for (int i = 1; i < rows.size(); i++) {    //first row is the col names so skip it.
                               ...
                               for (int i=0; i<curr_num_classes; i++){
                                   if(classList.get(i).length() > 1 && profList.get(i).length() > 1)
                                       setText = setText + "\nClass "+ (i+1) + " " + classList.get(i) + " with " + profList.get(i) + " : " + workHours.get(0);
                                   else if(classList.get(i).length() > 1)
                                       setText = setText + "\nClass "+ (i+1) + " " + classList.get(i) +  " : " + workHours.get(0);
                               }
